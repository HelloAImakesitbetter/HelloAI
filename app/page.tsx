  ...
  return (
    <main className="command-shell">
      ...
      <section className="command-main">
        ...
        {result ? <section className="project-board"><div className="board-heading"><div><span className="eyebrow">ACTIVE PROJECT</span><h2>{businessName || "Untitled project"}</h2></div><span className="draft-pill">In progress</span></div><div className="project-board-grid"><div><pre className="script-preview">{result}</pre><div className="tool-row"><button type="button" onClick={generateScenes} disabled={isGeneratingScenes} className="secondary-button">▧ {isGeneratingScenes ? "Creating scenes..." : "Create scenes"}</button><a href="/video" className="primary-button small">Open Video Studio →</a></div>{aiVideoStatus && <div className="progress-note">{aiVideoStatus}</div>}</div><div className="project-steps"><div className={result ? "plan-step done" : "plan-step current"}><span>01</span><div><strong>Shape the idea</strong><small>Script generated</small></div><b>✓</b></div><div className={scenes.length === 3 ? "plan-step done" : "plan-step"}><span>02</span><div><strong>Set the scene</strong><small>{scenes.length === 3 ? "Three scenes ready" : "Create visual direction"}</small></div><b>{scenes.length === 3 ? "✓" : "·"}</b></div><div className={aiVideoUrl || videoUrl ? "plan-step done" : "plan-step"}><span>03</span><div><strong>Make it talk</strong><small>{aiVideoUrl || videoUrl ? "Lip-synced video ready" : "Open Video Studio"}</small></div><b>{aiVideoUrl || videoUrl ? "✓" : "·"}</b></div></div></div></section> : <section className="next-actions"><div><span className="eyebrow">START HERE</span><h2>One brief can become a whole business kit.</h2><p>Begin with the idea above, then use the workspace to create the pieces around it.</p></div><div className="action-links"><a href="/video"><span>▶</span><strong>Make a video</strong><small>Turn the offer into a talking production.</small></a><a href="/website"><span>▤</span><strong>Build a website</strong><small>Turn the offer into a home online.</small></a><a href="/characters"><span>◉</span><strong>Set up your cast</strong><small>Give your videos faces and voices.</small></a></div></section>}
