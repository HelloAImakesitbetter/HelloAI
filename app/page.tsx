  ...
  return (
    <main className="task-workspace-shell">
      ...
      <section className="task-workspace-main">
        ...
        <section className="task-hero"><span className="eyebrow">{mode === "build" ? "BUILD MODE" : "GROWTH MODE"}</span><h1>What needs to be done?</h1><p className="task-hero-intro">Tell HelloAI what you are trying to achieve. It will recommend the best next step before taking you there.</p><div className="task-composer"><textarea value={task} onChange={(event) => setTask(event.target.value)} placeholder={mode === "build" ? "Tell me what you want to make or improve..." : "Tell me how you want to grow your business..."} /><div className="task-composer-footer"><button type="button">＋</button><span>{mode === "grow" ? "Growth agent" : "HelloAI agent"}⌄</span><span className="task-composer-spacer" /><button type="button">Auto⌄</button><button className="task-submit" type="button" onClick={submitTask} disabled={!task.trim() || isThinking}>{isThinking ? "…" : "→"}</button></div></div>{recommendation && <div className="task-recommendation"><span className="eyebrow">HELLOAI RECOMMENDS</span><p>{recommendation}</p><a className="primary-button" href={recommendationHref}>{recommendationLabel} →</a></div>}</section>
